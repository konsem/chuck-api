...
public class SlackController {
    ...
    public @RequestMapping(value = "/connect/slack", method = RequestMethod.GET, headers = HttpHeaders.ACCEPT + "="
            + MediaType.TEXT_HTML_VALUE, produces = MediaType.TEXT_HTML_VALUE) ModelAndView connect(
     ...
     */
    public @ResponseBody @RequestMapping(value = { "/integration/slack", "/jokes/slack" }, method = RequestMethod.POST, headers = {
        ...
        if (request.getText().startsWith("?")) {
            ...
            String[] categories = jokeRepository.findAllCategories();
            String[] whitelistedCategories = slackService.filterNonWhitelistedCategories(categories);
            Page<Joke> jokes = jokeService.searchWithCategoryFilter(query, whitelistedCategories, pageable);
            ...
            SlackCommandResponseAttachment[] attachments = new CommandResponseAttachment[jokes.getContent().size()];
