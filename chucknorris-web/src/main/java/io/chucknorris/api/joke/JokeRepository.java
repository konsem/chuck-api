...
public interface JokeRepository extends JpaRepository<Joke, String> {
  ...
  @Query(
      value =
          "SELECT j.categories, j.created_at, j.joke_id, j.updated_at, j.value "
              + "FROM joke AS j "
  ...
  @Query(
      ...
      countQuery =
          "SELECT count(*) " + "FROM find_by_by_value_contains_and_filter(:query, :categories)",
      nativeQuery = true)
  ...
  @Query(
      value =
          "SELECT j.categories->>0 FROM joke j WHERE j.categories IS NOT NULL "
  ...
  @Query(
      value =
          "WITH joke AS( SELECT joke_id, ROW_NUMBER() OVER (ORDER BY joke.created_at ASC,"
